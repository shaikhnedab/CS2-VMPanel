 ...
 */
  ...
  /**
 * Insert a new bundle (bundle + relations committed atomically)
 */
  insertNewPanelBundle: function (dataObj) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await db.withTransaction(async (exec, conn) => {
          ...
          const [qres] = await conn.query(query);
          const bundleInsertId = qres.insertId;

          const relRows = serverArray.map((s) => [bundleInsertId, s.split(':')[1]]);
          if (relRows.length) {
            const relQuery = db.queryFormat(`INSERT INTO ${relTable} (bundle_id,server_id) VALUES ?`, [relRows]);
            const relRes = await exec(relQuery, true);
            if (!relRes) throw new Error("Error in insertion bundle relation");
          }
        });
 ...
 */
  ...
  /**
* Delete a Bundle (relations removed atomically with the bundle row)
*/
    ...
    return new Promise(async (resolve, reject) => {
      try {
        ...
        await db.withTransaction(async (exec) => {
          const query = db.queryFormat(`DELETE FROM ${table} WHERE id = ? AND bundle_name = ?`, [dataObj.id, dataObj.bundlename]);
          const queryRes = await exec(query, true);
          if (!queryRes) throw new Error("Error in delete");

          const relQuery = db.queryFormat(`DELETE FROM ${relTable} WHERE bundle_id = ?`, [dataObj.id]);
          const relRes = await exec(relQuery, true);
          if (!relRes) throw new Error("Error in delete");
        });
