 ...
 */
  ...
  /**
 * get all the servers (full rows — admin/sale listings must project explicitly)
 */
  ...
  /**
 * get sale-ready server rows WITHOUT secrets (rcon never leaves the DB)
 */
  getPanelServersSaleListing: function () {
    return new Promise(async (resolve, reject) => {
      try {

        let query = db.queryFormat(`SELECT id, tbl_name, server_name, server_ip, server_port,
                                           vip_slots, vip_price, vip_currency, vip_flag, vip_days, created_at
                                    FROM ${table}`);
        let queryRes = await db.query(query);
        if (!queryRes) {
          return reject("No Data Found");
        ...
        logger.error("error in getPanelServersSaleListing->", error);
        reject(error)
      }
    });
  },
