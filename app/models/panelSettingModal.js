...
// Short-lived settings cache: settings are read on every request for the
// layout. Cache for 5s so a settings save propagates almost immediately
// while sparing the DB a full-table read per request.
let settingsCache = {
  data: null,
  fetchedAt: 0,
};
 ...
 */
  ...
  /**
   * get all the settings data form the table (5s cached)
   */
  getAllSettings: function (forceRefresh) {
    return new Promise(async (resolve, reject) => {
      try {
        const now = Date.now();
        if (!forceRefresh && settingsCache.data && (now - settingsCache.fetchedAt) < CACHE_TTL_MS) {
          return resolve({ ...settingsCache.data });
        }
  ...
  updateSetting: function (key, value) {
    return new Promise(async (resolve, reject) => {
      try {
        ...
        // Invalidate the cache immediately so the UI reflects the change.
