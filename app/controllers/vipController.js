...
exports.dashboard = async (req, res) => {
  try {
    // Dashboard is private: guests (no panel session, no Steam session) go to login.
    if (!req.session.token && !req.session.passport) return res.redirect('/login');
    const token = req.session.token
    let result = await dashboardFunc(req.body, token);
    // Nudge Steam players who hold no VIP anywhere toward the store.
    result.myVipCount = null;
    if (req.session.passport && !req.session.token) {
      try {
        const SteamIDConverter = require('../utils/steamIdConvertor');
        const steamId = SteamIDConverter.toSteamID(req.session.passport.user.id);
        const owned = await myDashboardModel.getUserDataFromAllServers('"' + steamId + '"');
        result.myVipCount = (owned || []).filter((r) => r && r.data).length;
      } catch (e) {
        logger.error("error counting user VIPs->", e);
        result.myVipCount = null;
      }
    }
...
exports.getVipsDataSingleServer = async (req, res) => {
  try {
    ...
    return sendSafeError(req, res, error, "Failed to load VIP listing.");
  }
}
...
exports.getAdminsDataSingleServer = async (req, res) => {
  try {
    ...
    return sendSafeError(req, res, error, "Failed to load admin listing.");
  }
}
