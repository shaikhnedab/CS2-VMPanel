...
//-----------------------------------------------------------------------------------------------------
// Delegated slot init: <div class="vmp-paypal-slot" id=".." data-payload=".." data-buytype="..">

function vmpInitPayPalSlot(el) {
  if (!el) return;
  try {
    var data = JSON.parse(decodeURIComponent(el.getAttribute('data-payload') || '%7B%7D'));
    setPayPalButton(el.id, data, el.getAttribute('data-buytype') || 'newPurchase');
  ...
  if (paypalActive == true && price && currency && server) {
    paypal.Buttons({
      ...
      onApprove: async (data, actions) => {
        ...
        var gift = (typeof vmpGetGiftFields === 'function') ? vmpGetGiftFields() : null;
        if (gift === null) { $("#divForLoader").html(""); return; }
        var payload = {
