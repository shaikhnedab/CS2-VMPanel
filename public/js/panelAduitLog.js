  ...
  fetch('/fetchauditlogs', {
    ...
    .then((response) => {
      ...
      for (let i = 0; i < dataArray.length; i++) {
        const a = dataArray[i];
        const esc = (v) => String(v == null ? 'NA' : v).replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' })[c]);
        htmlString += `<tr>
          <td><span class="vmp-badge violet">${esc(a.activity)}</span></td>
          <td class="truncate" style="max-width:320px" title="${esc(a.additional_info)}">${esc(a.additional_info)}</td>
          <td class="truncate" style="max-width:180px">${esc(a.created_by)}</td>
          <td class="mono tnum" style="white-space:nowrap">${a.created_at ? dateFormatter(a.created_at) : 'NA'}</td>
        </tr>`
      }
