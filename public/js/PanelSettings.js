...
function addNewPAdminajax() {
  if (curentAdminType === 1) {
    ...
    if (!$('#username_padd').val()) {
      formError = "Admin username is required"
    } else if (!$('#password_padd').val()) {
      formError = "Password is required"
    } else if (!$('#admintype_padd').val()) {
      formError = "Choose an admin type"
    }
...
function manuallyRefreshAllServerajax() {
  ...
  if (curentAdminType === 1) {
    ...
    let htmlString = `<p>Refresh all servers now?</p><p class="vmp-hint">Removes expired VIPs and pushes fresh configs to every server.</p>`
